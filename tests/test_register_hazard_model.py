...
SCRIPT_PATH = REPO_ROOT / "tools" / "register_hazard_model.py"


def _load_script():
    spec = importlib.util.spec_from_file_location("register_hazard_model", SCRIPT_PATH)
...
def test_register_hazard_model_copies_artifact_and_writes_manifest():
    script = _load_script()
    workspace = REPO_ROOT / "cache" / "pytest" / "test_register_hazard_model" / uuid4().hex
    ...
    result = script.register_hazard_model(config)

    assert result.artifact_path.exists()
    assert result.artifact_path.read_bytes() == b"fake-hazard-yolo-weights"
    ...
    assert manifest.model_id == "smartcart_hazard_yolov8n_test_pt_v1"
...
def test_register_hazard_model_dry_run_does_not_write_outputs(capsys):
    script = _load_script()
    workspace = REPO_ROOT / "cache" / "pytest" / "test_register_hazard_model_dry_run" / uuid4().hex
    source_model = workspace / "run" / "weights" / "best.pt"
    weights_dir = workspace / "weights"
    manifest_dir = workspace / "manifests"
    source_model.parent.mkdir(parents=True, exist_ok=True)
    ...
    config = script.RegisterConfig(
        ...
        weights_dir=weights_dir,
        manifest_dir=manifest_dir,
        model_id="smartcart_hazard_yolov8n_dryrun_pt_v1",
    )
    result = script.register_hazard_model(config, dry_run=True)
    captured = capsys.readouterr()

    assert "hazard_model_registration_plan" in captured.out
    assert "status=dry_run" in captured.out
    assert not result.artifact_path.exists()
    assert not result.manifest_path.exists()


def test_register_hazard_model_cli_dry_run(monkeypatch, capsys):
    script = _load_script()
    workspace = REPO_ROOT / "cache" / "pytest" / "test_register_hazard_model_cli" / uuid4().hex
    source_model = workspace / "run" / "weights" / "best.pt"
    source_model.parent.mkdir(parents=True, exist_ok=True)
    source_model.write_bytes(b"fake-hazard-yolo-weights")
    monkeypatch.setattr(
        ...
        [
            ...
            "--model-id",
            "smartcart_hazard_yolov8n_cli_pt_v1",
            "--dry-run",
        ],
    )
