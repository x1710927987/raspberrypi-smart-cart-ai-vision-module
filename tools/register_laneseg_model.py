...
DEFAULT_RUN_DIR = REPO_ROOT / "models" / "training" / "laneseg_yolo_v1" / "smartcart_laneseg_yolov8n_seg_roboflow_v1"
DEFAULT_SOURCE_MODEL = DEFAULT_RUN_DIR / "weights" / "best.pt"
DEFAULT_TEMPLATE = REPO_ROOT / "models" / "model_manifest.laneseg.example.json"
DEFAULT_WEIGHTS_DIR = REPO_ROOT / "models" / "weights"
DEFAULT_MANIFEST_DIR = REPO_ROOT / "models" / "training"
DEFAULT_MODEL_ID = "smartcart_laneseg_yolov8n_seg_roboflow_pt_v1"
DEFAULT_DATASET_MANIFEST = "data/external/roboflow_sidewalk_v1_split/data.yaml"
...
class RegisterConfig:
    ...
    version: str = "v1"
    dataset_manifest: str = DEFAULT_DATASET_MANIFEST
    dataset_version: str = "roboflow_sidewalk_v1_split"
    license: str = "unknown"
...
def register_laneseg_model(config: RegisterConfig, *, dry_run: bool = False) -> RegisterResult:
...
def build_manifest(
    *,
    ...
    payload["task"] = "laneseg"
    payload["status"] = "registered"
    payload["description"] = f"Registered lane/drivable-area YOLO segmentation artifact from {config.run_dir or config.source_model.parent}."
    ...
    source["dataset_manifest"] = config.dataset_manifest
    ...
    notes.append("Generated by tools/register_laneseg_model.py.")
...
def print_registration_plan(source_model: Path, artifact_path: Path, manifest_path: Path, sha256: str, size_bytes: int) -> None:
    print("laneseg_model_registration_plan")
...
def _read_final_metrics(run_dir: Path | None) -> dict[str, float | None]:
    ...
    return {
        "mask_map50": _float_or_none(final.get("metrics/mAP50(M)")),
        "mask_map50_95": _float_or_none(final.get("metrics/mAP50-95(M)")),
        "mask_precision": _float_or_none(final.get("metrics/precision(M)")),
        "mask_recall": _float_or_none(final.get("metrics/recall(M)")),
        "box_map50": _float_or_none(final.get("metrics/mAP50(B)")),
        "latency_ms": None,
    }
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Register a trained lane/drivable-area YOLO segmentation artifact.")
    ...
    parser.add_argument("--architecture", default="yolov8n-seg")
    parser.add_argument("--version", default="v1")
    parser.add_argument("--dataset-manifest", default=DEFAULT_DATASET_MANIFEST)
    parser.add_argument("--dataset-version", default="roboflow_sidewalk_v1_split")
...
def main() -> int:
    ...
    try:
        register_laneseg_model(_config_from_args(args), dry_run=args.dry_run)
