...
DEFAULT_RUN_DIR = REPO_ROOT / "models" / "training" / "hazard_yolo_v1" / "smartcart_hazard_yolov8n_roboflow_v1"
DEFAULT_SOURCE_MODEL = DEFAULT_RUN_DIR / "weights" / "best.pt"
DEFAULT_TEMPLATE = REPO_ROOT / "models" / "model_manifest.hazard.example.json"
DEFAULT_WEIGHTS_DIR = REPO_ROOT / "models" / "weights"
DEFAULT_MANIFEST_DIR = REPO_ROOT / "models" / "training"
DEFAULT_MODEL_ID = "smartcart_hazard_yolov8n_roboflow_pt_v1"
DEFAULT_DATASET_MANIFEST = "data/external/roboflow_hazard_v1_split/data.yaml"
...
class RegisterConfig:
    ...
    dataset_version: str = "roboflow_hazard_v1_split"
    license: str = "unknown"
...
def register_hazard_model(config: RegisterConfig, *, dry_run: bool = False) -> RegisterResult:
...
def build_manifest(
    *,
    ...
    payload["description"] = f"Registered hazard YOLO artifact from {config.run_dir or config.source_model.parent}."
    ...
    model_classes = _read_dataset_classes(config.dataset_manifest)
    if model_classes:
        payload["model_classes"] = model_classes
        payload["schema_mapping"] = {label: label for label in model_classes}
    ...
    if not isinstance(notes, list):
        notes = []
    notes = [str(item) for item in notes if "Replace example" not in str(item)]
    notes.append("Generated by tools/register_hazard_model.py.")
...
def print_registration_plan(source_model: Path, artifact_path: Path, manifest_path: Path, sha256: str, size_bytes: int) -> None:
    print("hazard_model_registration_plan")
...
def _read_dataset_classes(dataset_manifest: str) -> list[str]:
    path = Path(dataset_manifest)
    if not path.is_absolute():
        path = REPO_ROOT / path
    if not path.exists():
        return []
    return read_yolo_config(path).names
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Register a trained hazard YOLO artifact.")
    ...
    parser.add_argument("--dataset-version", default="roboflow_hazard_v1_split")
...
def main() -> int:
    ...
    try:
        register_hazard_model(_config_from_args(args), dry_run=args.dry_run)
