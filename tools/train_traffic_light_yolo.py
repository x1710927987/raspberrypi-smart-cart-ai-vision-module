...
DEFAULT_CONFIG = REPO_ROOT / "models" / "training" / "traffic_light_yolo_v1.yaml"
DEFAULT_DATA = REPO_ROOT / "data" / "external" / "roboflow_traffic_light_v1_split" / "data.yaml"
DEFAULT_PROJECT = REPO_ROOT / "models" / "training" / "traffic_light_yolo_v1"
DEFAULT_NAME = "smartcart_traffic_light_yolov8n_roboflow_v1"
...
class TrainConfig:
    ...
    manifest_template: Path = REPO_ROOT / "models" / "model_manifest.traffic_light.example.json"
    notes: str = "Traffic-light YOLO v1 baseline."
...
def train_traffic_light_yolo(config: TrainConfig, *, dry_run: bool = False) -> Path | None:
...
def print_training_plan(config: TrainConfig) -> None:
    print("traffic_light_yolo_training_plan")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the traffic-light YOLO baseline model.")
...
def _merge_cli_args(config: TrainConfig, args: argparse.Namespace) -> TrainConfig:
    updates: dict[str, Any] = {}
    for field_name in ("data", "model", "epochs", "imgsz", "batch", "device", "workers", "patience", "seed", "project", "name", "export_format"):
...
def main() -> int:
    ...
    try:
        ...
        train_traffic_light_yolo(config, dry_run=args.dry_run)
