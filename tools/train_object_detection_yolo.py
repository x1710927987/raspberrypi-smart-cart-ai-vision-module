...
DEFAULT_CONFIG = REPO_ROOT / "models" / "training" / "object_detection_yolo_v1.yaml"
DEFAULT_DATA = REPO_ROOT / "data" / "processed" / "objects_yolo_v1" / "data.yaml"
DEFAULT_PROJECT = REPO_ROOT / "models" / "training" / "object_detection_yolo_v1"
DEFAULT_NAME = "smartcart_objects_yolov8n_roboflow_v1"
...
class TrainConfig:
    ...
    manifest_template: Path = REPO_ROOT / "models" / "model_manifest.objects.example.json"
    notes: str = "Object-detection YOLO v1 baseline for pedestrian, bicycle, and car."
...
def train_object_detection_yolo(config: TrainConfig, *, dry_run: bool = False) -> Path | None:
...
def print_training_plan(config: TrainConfig) -> None:
    print("object_detection_yolo_training_plan")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the object-detection YOLO baseline model.")
...
def main() -> int:
    ...
    try:
        ...
        train_object_detection_yolo(config, dry_run=args.dry_run)
