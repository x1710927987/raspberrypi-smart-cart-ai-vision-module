...
DEFAULT_CONFIG = REPO_ROOT / "models" / "training" / "laneseg_yolo_v1.yaml"
DEFAULT_DATA = REPO_ROOT / "data" / "external" / "roboflow_sidewalk_v1_split" / "data.yaml"
DEFAULT_PROJECT = REPO_ROOT / "models" / "training" / "laneseg_yolo_v1"
DEFAULT_NAME = "smartcart_laneseg_yolov8n_seg_roboflow_v1"


@dataclass(frozen=True)
class TrainConfig:
    data: Path = DEFAULT_DATA
    model: str = "models/weights/yolov8n-seg.pt"
    epochs: int = 100
    imgsz: int = 640
    batch: int = 16
    ...
    manifest_template: Path = REPO_ROOT / "models" / "model_manifest.laneseg.example.json"
    notes: str = "Lane/drivable-area YOLO segmentation v1 baseline for sidewalk masks."
...
def train_laneseg_yolo(config: TrainConfig, *, dry_run: bool = False) -> Path | None:
...
def print_training_plan(config: TrainConfig) -> None:
    print("laneseg_yolo_training_plan")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the lane/drivable-area YOLO segmentation baseline model.")
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG, help="Flat YAML training config.")
    parser.add_argument("--data", type=Path, help="YOLO segmentation data.yaml path.")
...
def main() -> int:
    ...
    try:
        ...
        train_laneseg_yolo(config, dry_run=args.dry_run)
