...
DEFAULT_RUN_DIR = REPO_ROOT / "models" / "training" / "traffic_light_yolo_v1" / "smartcart_traffic_light_yolov8n_roboflow_v1"
DEFAULT_SOURCE_MODEL = DEFAULT_RUN_DIR / "weights" / "best.pt"
DEFAULT_TEMPLATE = REPO_ROOT / "models" / "model_manifest.traffic_light.example.json"
DEFAULT_WEIGHTS_DIR = REPO_ROOT / "models" / "weights"
DEFAULT_MANIFEST_DIR = REPO_ROOT / "models" / "training"
DEFAULT_MODEL_ID = "smartcart_traffic_light_yolov8n_roboflow_pt_v1"
...
class RegisterConfig:
    ...
    version: str = "v1"
    dataset_version: str = "roboflow_traffic_light_v1_split"
    license: str = "unknown"
...
def register_traffic_light_model(config: RegisterConfig, *, dry_run: bool = False) -> RegisterResult:
...
def build_manifest(
    *,
    ...
    payload["task"] = "traffic_light"
    payload["status"] = "registered"
    payload["description"] = f"Registered traffic-light YOLO model artifact from {config.run_dir or config.source_model.parent}."
    ...
    source["dataset_manifest"] = "data/external/roboflow_traffic_light_v1_split/data.yaml"
    ...
    notes.append("Generated by tools/register_traffic_light_model.py.")
...
def print_registration_plan(source_model: Path, artifact_path: Path, manifest_path: Path, sha256: str, size_bytes: int) -> None:
    print("traffic_light_model_registration_plan")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Register a trained traffic-light YOLO artifact into models/weights and a model manifest.")
    ...
    parser.add_argument("--dataset-version", default="roboflow_traffic_light_v1_split")
...
def main() -> int:
    ...
    try:
        register_traffic_light_model(_config_from_args(args), dry_run=args.dry_run)
